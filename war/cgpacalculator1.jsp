...
<br>
<form name="frm" method="post" action="cgpacalculator1.jsp" >
    ...
    <tbody>
                ...
                <tr>
                    <td>semester 1 </td>
					<td><input type="text" name="sgpa1" pattern= "[0-9]{1}[0]?(\.[0-9][0-9]?)?" title="SGPA format X.XX"  onkeypress="return isNumber(event)" required></td>
					<td><input type="text"  name="totcrd1" pattern= "^[+]?([0-9]+(?:[\.][0-9]*)?|\.[0-9]+)$" title="Enter valid data" onkeypress="return isNumber(event)"  required></td>
					<td><input type="text"  name="crdern1" pattern= "^[+]?([0-9]+(?:[\.][0-9]*)?|\.[0-9]+)$" title="Enter valid data" onkeypress="return isNumber(event)" pattern= "^(?=.*\d)\d*(?:\.\d\d)?$" required></td>
</tr>
  <%
                            for(int i=2;i<=sem;i++)
                            {
					...
					<td><input type="text"  name="totcrd<%=i%>" pattern= "^[+]?([0-9]+(?:[\.][0-9]*)?|\.[0-9]+)$" title="Enter valid data" onkeypress="return isNumber(event)" required></td>
					<td><input type="text"  name="crdern<%=i%>" pattern= "^[+]?([0-9]+(?:[\.][0-9]*)?|\.[0-9]+)$" title="Enter valid data" onkeypress="return isNumber(event)" required></td>
</tr>
 ...
 <!-- ============ FOOTER SECTION ============== -->
<tr><td colspan="2" align="left" height="20" bgcolor="#AFBCE8"></td></tr>
</table>
